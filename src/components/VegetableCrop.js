  ...
  useEffect(() => {
    if (selectedDistrict) {
      const fetchDistrictData = async () => {
        setLoading(true);
        try {
          const url = `http://127.0.0.1:8000/api/api/district-data/${selectedDistrict}/vege-crop/`;
          const response = await fetch(url);
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
  ...
  useEffect(() => {
    if (selectedCenter) {
      const fetchCenterCropData = async () => {
        setLoading(true);
        try {
          const url = `http://127.0.0.1:8000/api/api/get-centers-crop/${selectedCenter}/vege-crop`;
          const response = await fetch(url);
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
