  ...
  useEffect(() => {
    if (selectedDistrict) {
      const fetchDistrictData = async () => {
        setLoading(true);
        try {
          const url = `http://34.225.2.83:8000/api/api/district-data/${selectedDistrict}/vege-crop/`;
          const response = await fetch(url);
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
          ...
          if (data.crop_datas_list && data.crop_datas_list.length === 0) {
            ...
            setVegetableCrops([]);  // Clear crops when no data is returned
          } else {
            setVegetableCrops(data.crop_datas_list || []); // Set crops data
          }
      ...
      setVegetableCrops([]); // Clear the crops if no district is selected
    }
  }, [selectedDistrict]);
  ...
  useEffect(() => {
    if (selectedCenter) {
      const fetchCenterCropData = async () => {
        setLoading(true);
        try {
          const url = `http://34.225.2.83:8000/api/api/get-centers-crop/${selectedCenter}/vege-crop`;
          const response = await fetch(url);
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
          const data = await response.json();
          setVegetableCrops(data.crop_datas_list || []); // Update crops with service center data
        } catch (error) {
          console.error('Error fetching center crop data:', error.message);
          setVegetableCrops([]); // Clear crops on error
        } finally {
          setLoading(false);
        }
      };
