  ...
  useEffect(() => {
    if (selectedDistrict) {
      const fetchDistrictData = async () => {
        setLoading(true);
        try {
          const url = `http://34.225.2.83:8000/api/api/district-data/${selectedDistrict}/fruit-crop/`;
          const response = await fetch(url);
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
  ...
  useEffect(() => {
    if (selectedCenter) {
      const fetchCenterCropData = async () => {
        setLoading(true);
        try {
          const url = `http://34.225.2.83:8000/api/api/get-centers-crop/${selectedCenter}/fruit-crop`;
          const response = await fetch(url);
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
